(function () {
   ...
   // Populate sampling year selection
   function populateSampleYearDropdown(datatype) {
      
      const firstYear = 1999
      const currentYear = parseInt(new Date().getFullYear())

      // Create array of year since first year
      let yearsArray = []
      for (let year = firstYear; year <= currentYear; year++) {
         yearsArray = [year, ...yearsArray]
      }

      // Grab dropdown and populate it with options for each year
      // Each datatype has its own dropdown, so they need to be grabbed dynamically
      let loginYearDropdown = document.querySelector(`#${datatype}-year-select`)
      yearsArray.forEach(year => {
         loginYearDropdown.innerHTML += `<option value=${year}>${year}</option>`
      })
   }

   // Call function on load for algae, the first datatype in the list
   populateSampleYearDropdown('algae') 
   // Listen for change in datatype and call the function again if it changes
   document.querySelector("#datatype-select").addEventListener("change", (e) => {
      let datatype = e.target.value
      populateSampleYearDropdown(datatype)
   })
})()