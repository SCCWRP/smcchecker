...
def phab(all_dfs):
    ...
    print("# CHECK - 2")
    # Description: Actual longitude should not be 300m away from target longitude (🛑 Warning 🛑)
    # Created Coder: Aria Askaryar
    # Created Date: 10/17/2023
    # Last Edited Date: 10/17/2023
    # Last Edited Coder: Aria Askaryar
    ...
    errs.append(
        checkData(
            'tbl_phab',
            merged_df[merged_df['longitude_difference'] > distance_threshold].index.tolist(),
            'actual_longitude',
            'Undefined Error',
            'Actual longitude should not be 300m away from target longitude.'                  
        )
    )
